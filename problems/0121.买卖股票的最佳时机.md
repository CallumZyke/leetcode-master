...
//关于贪心：
// 最低价在早期出现，而最高利润在之后计算得出。由此想到，遍历过程中先抓到目前碰到过的最小值，然后用这个最小值来当天卖掉，和记录的最大值比较即可

//关于动态规划（包含一次和多次，和最大交易次数k）：[见这个网站](https://labuladong.online/algo/dynamic-programming/stock-problem-summary/#二、状态转移框架)这个框架好！ 注意，1次交易包括了buy和sell, 而对于dp[i][k][0]之dp[i-1][k][1]+prices[i]这里是k，因为这个股票要卖掉才能用掉一次完整的交易机会,产生一笔transaction fee。

// dp[...][0][0]直接算成0 
// dp[...][0][1] = INT_MIN

// vector<vector<vector<int>>> dp(n, vector<vector<int>>(k + 1, vector<int>(2)));
// 中的vector<vector<int>>(k+1, vector<int>(2))就是标准的vector声明初始化数组的写法。规律是vector数量:3->2->1
...
//买可能受到最大交易次数以及冷冻期的限制，故改变的都是包含了-prices[i]的那一行
